...
#      cl_output('filesystem_image_exists '.$docroot.' '.$filename);
        ...
        return
            is_file($_SERVER['DOCUMENT_ROOT'].$GLOBALS['pageroot'].'/'.Config::UPLOADIMAGES_DIR.'/image/'.$localfile)
            || is_file($_SERVER['DOCUMENT_ROOT'].$GLOBALS['pageroot'].'/'.Config::UPLOADIMAGES_DIR.'/'.$localfile)
            ## commandline
...
#      cl_output('get file system image'.$filename.' '.$localfile);
        if (Config::UPLOADIMAGES_DIR != '') {
            ...
            if (is_file($imageroot.$localfile)) {
                ...
                } elseif (is_file($_SERVER['DOCUMENT_ROOT'].'/'.Config::UPLOADIMAGES_DIR.'/image/'.$localfile)) {
        ...
        } elseif (is_file($_SERVER['DOCUMENT_ROOT'].$GLOBALS['pageroot'].'/'.Config::UPLOADIMAGES_DIR.'/'.$localfile)) {
            $elements = parse_url($filename);
            $localfile = basename($elements['path']);
            return base64_encode( file_get_contents($_SERVER['DOCUMENT_ROOT'].$GLOBALS['pageroot'].'/'.Config::UPLOADIMAGES_DIR.'/'.$localfile));
        } elseif (is_file($_SERVER['DOCUMENT_ROOT'].$GLOBALS['pageroot'].'/'.Config::UPLOADIMAGES_DIR.'/image/'.$localfile)) {
            return base64_encode( file_get_contents($_SERVER['DOCUMENT_ROOT'].$GLOBALS['pageroot'].'/'.Config::UPLOADIMAGES_DIR.'/image/'.$localfile));
    ...
    function image_exists($templateid,$filename) {
        ...
        $query = spirntf(
            ...
            Config::getTableName('templateimage'), $templateid, $filename, basename($filename));
        $rs = phpList::DB()->Sql_Query($query);
        return phpList::DB()->Sql_Num_Rows($rs);
    }

    function get_template_image($templateid,$filename){
        if (basename($filename) == 'powerphplist.png') $templateid = 0;
        $query = sprinf('SELECT data FROM %s
            WHERE template = %s
            AND (filename = "%s" or filename = "%s")',
            Config::getTableName('templateimage'), $templateid, $filename, basename($filename));

        $req = phpList::DB()->Sql_Fetch_Row_Query($query);
        return $req[0];
    }
    ...
    function AmazonSESSend($messageheader,$messagebody) {
        ...
        */
        ...
        curl_setopt($curl, CURLOPT_USERAGENT,NAME." (phpList version ".Config::get('VERSION').", http://www.phplist.com/)");
    ...
    public static function sendMail ($to, $subject, $message, $skipblacklistcheck = 0) {
        ...
        if (!$skipblacklistcheck && User::isBlackListed($to)) {
            ...
            addUserHistory(User::getUserByEmail($to)->id, 'Marked Blacklisted', 'Found user in blacklist while trying to send an email, marked black listed');
            return 0;
        }
