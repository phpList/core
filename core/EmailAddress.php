...
class EmailAddress implements interfaces\EmailAddressInterface {
    static $SKIP_VALIDATION = true;
    ...
    /**
     * Default constructor
     * When validation is not required, pass the skip parameter
     * @param Config $config
     * @param $email_address
     * @param bool $skip_validation
     */
    public function __construct(Config $config, $email_address, $skip_validation = false)
    {
        $this->config = $config;
        if($skip_validation){
            $this->email_address = $email_address;
        }else{
            $this->setAddress($email_address);
        }
    }
     ...
     * @return bool
     */
    private function isValid($email_address)
    {
        ...
        $ats = substr_count($email_address, '@');
        if ($ats != 1) {
            return false;
        }
        ...
        switch ($validation_level) {
            ...
            case 3: # RFC821 email validation.
                ...
                return (preg_match($validEmailPattern, $email_address) > 0);
                break;
            ...
            default: # 10.4 style email validation
                ...
                $pattern = "/^[\&\'-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+('.$allowed_tlds.')|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$/i";

                return (preg_match($pattern, $email_address) > 0);
                break;
        }
    }
