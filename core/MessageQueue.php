    ...
    public function process($message_id, $force = false, $reload = false, $cmd_max = 0){
        ...
        */
        ...
        if ($this->num_per_batch && $batch_period) {
            ...
            */
            $recently_sent = phpList::DB()->Sql_Fetch_Row_Query(sprintf(
                'SELECT COUNT(*) FROM %s
                WHERE entered > date_sub(CURRENT_TIMESTAMP,INTERVAL %d second)
                AND status = "sent"',
                Config::getTableName('usermessage'), $batch_period));
        ...
        if ($this->num_per_batch > 0) {
            ...
            if ($this->original_num_per_batch != $this->num_per_batch) {
                ...
                if ($diff < 0) $diff = 0;
                $this->output(s('This batch will be %d emails, because in the last %d seconds %d emails were sent',$this->num_per_batch,$this->batch_period, $diff),0,'progress');
            } else {
                $this->output(s('Sending in batches of %d emails',$this->num_per_batch),0,'progress');
            }
        } elseif ($this->num_per_batch < 0) {
            $this->output(s('In the last %d seconds more emails were sent (%d) than is currently allowed per batch (%d)', $this->batch_period,$recently_sent[0],$this->original_num_per_batch),0,'progress');
            $this->processed = 0;
            $this->script_stage = 5;
        ...
        if ($num_messages) {
            ...
            if (!Config::get('commandline', false) && empty($this->reload)) {
                if (!$safemode) {
                    $this->output(s('Please leave this window open. You have batch processing enabled, so it will reload several times to send the messages. Reports will be sent by email to').' '.getConfig("report_address"));
                } else {
                    $this->output(s('Your webserver is running in safe_mode. Please keep this window open. It may reload several times to make sure all messages are sent.').' '.s('Reports will be sent by email to').' '.getConfig("report_address"));
                }
            }
        }
        ...
        foreach ($messages as $message) {
            ...
            if ($message->getDataItem('resetstats') == 1) {
            ...
            $stopSending = false;
            //if (!empty($message->getDataItem('finishsending'))) {
                $finishSendingBefore = $message->getDataItem('finishsending')->getTimestamp();
                ...
                if (empty($this->reload)) {
                    ...
                    if (Config::VERBOSE) {
                        $this->output(sprintf(s('sending of this campaign will stop, if it is still going in %s'),secs2time($secondsTogo)));
                    }
                }
            //}

            $userselection = $message->getDataItem('userselection'); ## @@ needs more work
            ## load message in cache
            if (!precacheMessage($message->id)) {
            ...
            if (!empty($msgdata['notify_start']) && !isset($msgdata['start_notified'])) {
                ...
                foreach ($notifications as $notification) {
                    sendMail($notification,s('Campaign started'),
                        sprintf(s('phplist has started sending the campaign with subject %s'),$msgdata['subject']."\n\n".
                            sprintf(s('to view the progress of this campaign, go to http://%s'),getConfig('website').$GLOBALS['adminpages'].'/?page=messages&amp;tab=active')));
                }
            ...
            if (empty($reload)) {
                $this->output(s('Processing message').' '. $messageid);
            }

//  if(isset($GLOBALS['plugins']['rssmanager']) && $GLOBALS['plugins']['rssmanager']->enabled && $message["rsstemplate"]) {
// $processrss = 1;
//    $this->output(s('Message').' '. $messageid.' '.s('is an rss feed for').' '. s($rssmessage));
//  } else {
//    $processrss = 0;
//  }
            ...
            $findUserStart = $this->timer->elapsed(1);
            ...
            if ($userselection && $numattr[0]) {
                ...
                $this->counters['total_users_for_message'] = Sql_Num_Rows($res);
                if (empty($reload)) {
                    $this->output($this->counters['total_users_for_message'].' '.s('users apply for attributes, now checking lists'),0,'progress');
                }
                $user_list = "";
            ...
            */
            ...
            if (USE_LIST_EXCLUDE) {
                ...
                if (count($msgdata['excludelist'])) {
                    ...
                    if (VERBOSE) {
                        $this->output('Exclude query '.$query);
                    }
                    $req = Sql_Query($query);
                    while ($row = Sql_Fetch_Row($req)) {
            ...
            $this->counters['total_users_for_message '.$messageid] = Sql_Num_Rows($userids);
            if ($skipped >= 10000) {
                $this->counters['total_users_for_message '.$messageid] -= $skipped;
            }

            $findUserEnd = $this->timer->elapsed(1);

            if ($findUserEnd - $findUserStart > 300 && !$GLOBALS["commandline"]) {
                $this->output(s('Warning, finding the subscribers to send out to takes a long time, consider changing to commandline sending'));
            }

            if (empty($reload)) {
                $this->output(s('Found them').': '.$this->counters['total_users_for_message '.$messageid].' '.s('to process'));
            }
            setMessageData($messageid,'to process',$this->counters['total_users_for_message '.$messageid]);
            ...
            if (MAILQUEUE_BATCH_SIZE) {
                ...
                $batch_total = $this->counters['total_users_for_message '.$messageid];
                if ($this->num_per_batch > 0) {
            ...
            while ($userdata = Sql_Fetch_Row($userids)) {
                $this->counters['processed_users_for_message '.$messageid]++;
                $failure_reason = '';
...
# Throttling
                            ...
                            if (!isset($domainthrottle[$domainname]) || !is_array($domainthrottle[$domainname])) {
                                ...
                                if ($throttled) {
                                    ...
                                    if (DOMAIN_AUTO_THROTTLE
                                        ...
                                        && $this->counters['total_users_for_message '.$messageid] < 1000 # and also when there's not too many left, because then it's likely they're all being throttled
                                    ) {
                                        $domainthrottle[$domainname]['attempted'] = 0;
                                        logEvent(sprintf(s('There have been more than 10 attempts to send to %s that have been blocked for domain throttling.'),$domainname));
                                        ...
                                        $this->output(sprintf(s('%s is currently over throttle limit of %d per %d seconds').' ('.$domainthrottle[$domainname]['sent'].')',$domainname,DOMAIN_BATCH_SIZE,DOMAIN_BATCH_PERIOD));
                                    }
                                }
                            }
                        }
                        ...
                        if ($cansend) {
                            ...
                            if (!TEST) {
                                ...
                                if (!$throttled) {
                                    if (VERBOSE)
                                        $this->output(s('Sending').' '. $messageid.' '.s('to').' '. $useremail);
                                    ...
                                    if (VERBOSE) {
                                        $this->output(s('It took').' '.$emailSentTimer->elapsed(1).' '.s('seconds to send'));
                                    }
                                } else {
                                    $throttlecount++;
                                }
...
#             $this->output("Sent: $sent mph $msgperhour mps $msgpersec secpm $secpermsg target $target actual $actual d $delay");
                                ...
                                if ($delay > 0) {
                                    if (VERBOSE) {
                                        ...
                                        $this->output(sprintf(s('waiting for %.1f seconds to meet target of %s seconds per message'),
...
#          Sql_query("replace into {$tables['usermessage']} (userid,messageid) values($email[0],$messageid)");
                        ...
                        if (!$user['confirmed'] || $user['disabled']) {
                            if (VERBOSE)
                                $this->output(s('Unconfirmed user').': '.$userid.' '. $user['email'].' '.  $user['id']);
                            $unconfirmed++;
                    ...
                    $notsent++;
                    if (VERBOSE) {
                        $this->output(s('Not sending to').' '. $userdata[0].', '.s('already sent').' '.$um[0]);
                    }
                }
                ...
                #if ($processed % 10 == 0) {
                if (0) {
                    $this->output('AR'.$affrows.' N '.$this->counters['total_users_for_message '.$messageid].' P'.$processed.' S'.$sent.' N'.$notsent.' I'.$invalid.' U'.$unconfirmed.' C'.$cannotsend.' F'.$failed_sent);
                    $rn = $reload * $this->num_per_batch;
                    $this->output('P '.$processed .' N'. $this->counters['total_users_for_message '.$messageid] .' NB'.$this->num_per_batch .' BT'.$batch_total .' R'.$reload.' RN'.$rn);
                }
                /*
                ...
                if ($sent > 0) {
                    $msgperhour = (3600/$totaltime) * $sent;
                    $secpermsg = $totaltime / $sent;
                    $timeleft = ($this->counters['total_users_for_message '.$messageid] - $sent) * $secpermsg;
                ...
                setMessageData($messageid,'to process',$this->counters['total_users_for_message '.$messageid] - $sent);
                setMessageData($messageid,'last msg sent',time());
                #  setMessageData($messageid,'totaltime',$GLOBALS['processqueue_timer']->elapsed(1));
                if (!empty($getspeedstats)) $this->output('end process user '."\n".'-----------------------------------'."\n".$userid);
            }
            $processed = $notsent + $sent + $invalid + $unconfirmed + $cannotsend + $failed_sent;
            $this->output(s('Processed %d out of %d subscribers',$this->counters['processed_users_for_message '.$messageid],$this->counters['total_users_for_message '.$messageid]),1,'progress');

            if ($this->counters['total_users_for_message '.$messageid] - $sent <= 0 || $stopSending) {
                ...
                if (!$failed_sent) {
                    ...
                    if (!empty($msgdata['notify_end']) && !isset($msgdata['end_notified'])) {
                        ...
                        foreach ($notifications as $notification) {
                            sendMail($notification,s('Message campaign finished'),
                                sprintf(s('phpList has finished sending the campaign with subject %s'),$msgdata['subject'])."\n\n".
                                sprintf(s('to view the results of this campaign, go to http://%s'),getConfig('website').$GLOBALS['adminpages'].'/?page=statsoverview&id='.$messageid)
                            );
                        }
                    ...
                    $this->output(s('It took').' '.timeDiff($timetaken[0],$timetaken[1]).' '.s('to send this message'));
    ...
    public function shutdown(){
        ...
        if ($this->sent)
            $this->output(sprintf('%d %s %01.2f %s (%d %s)',$this->sent,s('messages sent in'),
                $totaltime,s('seconds'),$msgperhour,s('msgs/hr')),$this->sent,'progress');
        if ($this->invalid) {
        ...
        */
        ...
        if (!$this->nothingtodo) {
            ...
            printf('<script type="text/javascript">
                    ...
                    $GLOBALS['sent'], $this->counters['total_users_for_message ' . $this->messageid]);
        }
        //TODO:enable plugins
        /*
        ...
        if ($this->script_stage < 5 && !$this->nothingtodo) {
            ...
            if (!Config::get('commandline') && $this->num_per_batch && $this->batch_period) {
                if ($this->sent + 10 < $this->original_num_per_batch) {
                    ...
                    $this->output(sprintf(s('Waiting for %d seconds before reloading'),$this->batch_period),1,'progress');
                ...
                printf( '<script type="text/javascript">
                        ...
                        $this->reload, $this->sent, $this->notsent);
            } else {
                printf( '<script type="text/javascript">
                            document.location = "./?page=pageaction&action=processqueue&ajaxed=true&reload=%d&lastsent=%d&lastskipped=%d";
                         </script>',
    ...
    private function output ($message, $logit = 1, $target = 'summary') {
        ...
        if (Config::get('commandline')) {
            Util::cl_output(strip_tags($message).' ['.$this->timer->interval(1).'] ('.Config::get('pagestats')["number_of_queries"].')');
            ...
            foreach ($lines as $line) {
                ...
                printf('<script type="text/javascript">
                            ...
                            $target, ($this->shadecount) ? 1 : 0, $line, $target);
