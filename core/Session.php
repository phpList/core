...
class Session{
    ...
    private function checkMySQLSessionTable(){
        if (!phpList::DB()->Sql_Table_exists(Config::SESSION_TABLENAME)) {
            phpList::DB()->Sql_Create_Table(Config::SESSION_TABLENAME, array(
...
class MySQLSessionHandler /*implements \SessionHandlerInterface*/{
    ...
    {
        $retval = phpList::DB()->Sql_Query(sprintf(
            'DELETE FROM %s
            WHERE sessionid = \'%s\'',
            Config::SESSION_TABLENAME, addslashes($session_id)));
        return $retval;
    }
    ...
    public function gc($maxlifetime)
    {
        ...
        $retval = phpList::DB()->Sql_Query(sprintf(
            'DELETE FROM %s
            WHERE lastactive < %s',
            Config::SESSION_TABLENAME, $cutoff_time));
        return $retval;
    }
    ...
    {
        $session_data_req = phpList::DB()->Sql_Query(sprintf(
            'SELECT data FROM %s
            WHERE sessionid = \'%s\'',
            Config::SESSION_TABLENAME, addslashes($session_id)));
    ...
    public function write($session_id, $session_data)
    {
        ...
        $session_exists = phpList::DB()->Sql_Fetch_Row_Query(sprintf(
            'SELECT COUNT(*) FROM  %s
            WHERE sessionid = \'%s\'',
            Config::SESSION_TABLENAME, addslashes($session_id)));
        if ($session_exists[0] == 0) {
            $retval = phpList::DB()->Sql_Query(sprintf(
                ...
                Config::SESSION_TABLENAME, $session_id, $session_data));
        } else {
            $retval = phpList::DB()->Sql_Query(sprintf(
                'UPDATE %s
                ...
                WHERE sessionid = "%s"',
                Config::SESSION_TABLENAME, $session_id, $session_data));
            if (phpList::DB()->Sql_Affected_Rows() < 0) {
