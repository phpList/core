...
class Config {
    ...
    */
    ...
    const BOUNCE_PROTOCOL = 'pop';

    # set this to 0, if you set up a cron to download bounces regularly by using the
    # commandline option. If this is 0, users cannot run the page from the web
    # frontend. Read README.commandline to find out how to set it up on the
    # commandline
    const MANUALLY_PROCESS_BOUNCES = 1;

    # when the protocol is pop, specify these three
    const BOUNCE_MAILBOX_HOST = 'localhost';
    const BOUNCE_MAILBOX_USER = 'popuser';
    const BOUNCE_MAILBOX_PASSWORD = 'password';

    # the 'port' is the remote port of the connection to retrieve the emails
    # the default should be fine but if it doesn't work, you can try the second
    # one. To do that, add a # before the first line and take off the one before the
    # second line
    const BOUNCE_MAILBOX_PORT = '110/pop3/notls';
    #const BOUNCE_MAILBOX_PORT = '110/pop3';

    # it's getting more common to have secure connections, in which case you probably want to use
    #const BOUNCE_MAILBOX_PORT = '995/pop3/ssl/novalidate-cert';

    # when the protocol is mbox specify this one
    # it needs to be a local file in mbox format, accessible to your webserver user
    const BOUNCE_MAILBOX = '/var/spool/mail/listbounces';

    # set this to 0 if you want to keep your messages in the mailbox. this is potentially
    # a problem, because bounces will be counted multiple times, so only do this if you are
    # testing things.
    const BOUNCE_MAILBOX_PURGE = 1;

    # set this to 0 if you want to keep unprocessed messages in the mailbox. Unprocessed
    # messages are messages that could not be matched with a user in the system
    # messages are still downloaded into phpList, so it is safe to delete them from
    # the mailbox and view them in phpList
    const BOUNCE_MAILBOX_PURGE_UNPROCESSED = 1;

    # how many bounces in a row need to have occurred for a user to be marked unconfirmed
    const BOUNCE_UNSUBSCRIBE_THRESHOLD = 5;
    ...
    const UNSUBSCRIBE_REQUIRES_PASSWORD = 0;
    ...
    const WARN_ABOUT_PHP_SETTINGS = true;
    ...
    static $ALLOWED_REFERRERS = array();
    ...
    # const STATS_COLLECTION_ADDRESS = 'phplist-stats@phplist.com';
    ...
    const MANUALLY_PROCESS_QUEUE = true;
    ...
    const USE_DOMAIN_THROTTLE = false;
    const DOMAIN_BATCH_SIZE = 1;
    const DOMAIN_BATCH_PERIOD = 120;
    ...
    const DOMAIN_AUTO_THROTTLE = false;
    ...
    const DEFAULT_SYSTEM_LANGUAGE = 'en';
    ...
    const ALLOW_NON_LIST_SUBSCRIBE = false;
    ...
    const PREFERENCEPAGE_SHOW_PRIVATE_LISTS = false;
    ...
    const DATE_START_YEAR = 0;
    const DATE_END_YEAR = 0;
    ...
    const USE_ADMIN_DETAILS_FOR_MESSAGES = true;
    ...
    # 604800 - a week.
    # 86400 - a day
    const DEFAULT_MESSAGEAGE = 15768000;
    ...
    ## you have works ok
    #const PHPMAILER_PATH = '/usr/share/php/libphp-phpmailer/class.phpmailer.php';
    ...
    #const PHPMAILERHOST = 'smtp1.mydomain.com:25;smtp2.mydomain.com:2500;smtp3.phplist.com:5123';

    # if you want to use smtp authentication when sending the email uncomment the following
    # two lines and set the username and password to be the correct ones
    #const PHPMAILER_SMTPUSER = 'smtpuser';
    #const PHPMAILER_SMTPPASSWORD = 'smtppassword';

    ## you can set this to send out via a different SMTP port
    # const PHPMAILERPORT = 25;
    ...
    # const PHPMAILERTESTHOST = 'testsmtp.mydomain.com';
    # const PHPMAILERBLASTHOST = 'livesmtp.mydomain.com';
    # const PHPMAILERBLASTPORT = 25;

    # to use SSL/TLS when sending set this value
    # it can either be 'ssl' or 'tls', nothing else
    # const PHPMAILER_SECURE = 'ssl';
    ...
    const UPLOADIMAGES_DIR = 'uploadimages';

    ## for the above, you can also use subdirectories, for example
    #const UPLOADIMAGES_DIR = 'images/newsletter/uploaded';
    ...
    const USE_MANUAL_TEXT_PART = true;
    ...
    const ATTACHMENT_REPOSITORY = '/tmp';
    ...
    # or text/tsv
    const EXPORT_MIMETYPE = 'application/csv';
    ...
    const EMAIL_ADDRESS_VALIDATION_LEVEL = 2;
    ...
    # set to true to use
    const MESSAGEQUEUE_PREPARE = false;
    ...
    const STACKED_ATTRIBUTE_SELECTION = false;
    ...
    const USE_ADVANCED_BOUNCEHANDLING = false;
    ...
    const KEEPFORWARDERATTRIBUTES = false;
    ...
    const FORWARD_PERSONAL_NOTE_SIZE = false;
    ...
    const FORWARD_ALTERNATIVE_CONTENT = false;
    ...
    #const DATABASE_MODULE = 'adodb.inc';
    ...
    public static function fromDB($item_name, $default = null){
        if (Config::get('has_db_config')) {
            $query = sprintf(
                'SELECT value, editable
                FROM %s
                WHERE item = "%s"',
                Config::getTableName('config'), $item_name);
            $result = phpList::DB()->Sql_Query($query);
            if (phpList::DB()->Sql_Num_Rows($result) == 0) {
                if($default == null){
                    //try to get from default config
                    //TODO: remove globals
                    if (isset ($GLOBALS['default_config'][$item_name])) {
                        return $GLOBALS['default_config'][$item_name]['value'];
                    }
                }else{
                    return $default;
                }
    ...
    public static function setRunningConfig($key, $value){
        Config::instance()->running_config[$key] = $value;
    }

    public static function get($key){
        if(isset(Config::instance()->running_config[$key])){
            return Config::instance()->running_config[$key];
        }else{
            //try to find it in db
            return Config::fromDB($key);
        }
    }
